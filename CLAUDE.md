...
5. **Assets** (textures, models): only from the sources in doc 08, downloaded at dev time, **committed** to the repo. The **frontend** makes zero non-localhost requests. The backend's single allowed external request is the ISS TLE (CelesTrak, doc 02 step E): at most once per 24 h, committed fallback, a fetch failure never breaks the API.
